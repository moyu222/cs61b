...
//    }
    ...
    public boolean eachColExe(Board b, int col) {
        ...
        for (int currNum = b.size() - 1; currNum >= 0; currNum--) {
            ...
            if (target == null) {
                ...
                boolean flag = (target.col() != curr.col()) || (target.row() != curr.row());

                 if (curr.value() == target.value() && flag) {
                        // 如果当前棋子和目标棋子值相同，进行合并
                        b.move(col, aimRow, curr);
                        changeNum +=1;
                        this.score += b.tile(col, aimRow).value(); // 更新分数
                        aimRow--; // 合并后，更新目标行
                    } else {
                        // 否则，将当前棋子移动到目标行的上方
...
//        }
        for (int col = 0; col < this.board.size(); col++) {
            ...
            if (eachColExe(this.board, col)) {
                changed = true; // Set changed to true if any column changed
            }
